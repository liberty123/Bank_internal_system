...
class Predict:
    ...
    def __init__(self):
        ...
        self.Suggestion = "Recommendation not adopted"
        self.predictions = 0
        self.data = 0
    ...
    def model_predict(self, X_test):
        self.predictions = self.model.predict_classes(X_test)
        return 1
    ...
    def main(self):
        while True:
            ...
            for i in data:
                ...
                if not self.checking_data(i):
                    print("incomplete data, skip")
                    continue
                # build data from sql query result
                d = self.bulid_data(i)
                # preprocess data
                df = self.data_preprocess(d)
                X_test = df.values
                X_test = X_test.astype(np.float32)
                # predict
                self.model_predict(X_test)
                if self.predictions[0][0] == 1:
                    self.Suggestion = "Recommended to adopt"
                update_sqli = "update {} set loanstatus='{}' where id={}".format(table, self.Suggestion, i[0])
